...
;// CONCATENATED MODULE: ./src/lib/matching/product-matcher.ts
...
class ProductMatcher {
   ...
   */ async findCompatibleCases(gear, options = {}) {
        ...
        if (typeof gear === "string") {
            const foundGear = await gear_models/* AudioGear */.a1.findById(gear);
            if (!foundGear) {
        ...
        const cases = await gear_models/* Case */.JZ.find(query);
   ...
   */ async saveMatches(gear, cases) {
        ...
        if (bulkOps.length > 0) {
            await gear_models/* GearCaseMatch */.aX.bulkWrite(bulkOps);
        }
    }
    /**
   ...
   */ async findAudioGear(searchTerm, options = {}) {
        ...
        return gear_models/* AudioGear */.a1.find(query).limit(options.limit || 20).sort({
            popularity: -1
        });
    }
    /**
   ...
   */ async findCases(searchTerm, options = {}) {
        ...
        return gear_models/* Case */.JZ.find(query).limit(options.limit || 20).sort({
            rating: -1
        });
    }
    /**
   ...
   */ async getPopularMatches(limit = 10) {
        const matches = await gear_models/* GearCaseMatch */.aX.find().sort({
...
;// CONCATENATED MODULE: ./src/lib/matching/feature-matcher.ts
...
;// CONCATENATED MODULE: ./src/lib/matching/recommendation-engine.ts
...
;// CONCATENATED MODULE: ./src/pages/api/gear.ts
 ...
 */ async function handleGetRequest(req, res) {
    ...
    switch(action){
        case "all":
            ...
            const allGear = await gear_models/* AudioGear */.a1.find({
                ...category ? {
                    category
                } : {},
                ...brand ? {
        ...
        case "search":
            ...
            const searchResults = await gear_models/* AudioGear */.a1.find({
                $or: [
                    {
                        name: {
                            $regex: query,
        ...
        case "detail":
            ...
            const gearDetail = await gear_models/* AudioGear */.a1.findById(id);
            if (!gearDetail) {
        ...
        case "popular":
            ...
            const popularGear = await gear_models/* AudioGear */.a1.find().sort({
                popularity: -1
            }).limit(parseInt(limit));
 ...
 */ async function handlePostRequest(req, res) {
    ...
    switch(action){
        ...
        case "recommend-alternatives":
            ...
            try {
                const gear = await gear_models/* AudioGear */.a1.findById(gId);
                if (!gear) {
                    return res.status(404).json({
                        error: "Gear not found"
                    });
        ...
        case "add-gear":
            ...
            try {
                const createdGear = await gear_models/* AudioGear */.a1.create(newGear);
...
var __webpack_exports__ = __webpack_require__.X(0, [338], () => (__webpack_exec__(4148)));
